...
use crate::rpc_types::{RpcRequest, SubscribeParams, WaypointsInput};
...
pub fn dispatch_rpc(engine: &mut Engine, request: &[u8], out: &mut [u8]) -> (RpcAction, usize) {
    let msg: RpcRequest = match serde_json::from_slice(request) {
        Ok(m) => m,
        Err(_) => {
...
pub fn dispatch_parsed(
    ...
    match request.cmd.as_str() {
        ...
        "set-config" | "set_config" => match request.parse_params::<MotorControllerConfig>() {
        ...
        "append-waypoints" | "append_waypoints" => match request.parse_params::<WaypointsInput>() {
            Ok(input) => {
                let (waypoints, _) = input.into_parts();
                engine.apply(MotionCommand::AppendWaypoints(waypoints).into());
                respond(&id, "ok", out)
            }
            Err(_) => error(&id, -32602, "Invalid params", out),
        },
        "set-waypoints" | "set_waypoints" => match request.parse_params::<WaypointsInput>() {
            Ok(input) => {
        ...
        "reset-timestamp" | "reset_timestamp" => {
            engine.apply(MotionCommand::ResetTimestamp.into());
            respond(&id, "ok", out)
        }
