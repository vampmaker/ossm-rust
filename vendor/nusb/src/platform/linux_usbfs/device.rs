...
pub(crate) struct LinuxDevice {
    ...
    /// USBDEVFS mmap is EPERM on Android/Termux usbfs; skip after first failure.
    mmap_ok: AtomicBool,
}
...
impl LinuxEndpoint {
    ...
    pub(crate) fn allocate(&self, len: usize) -> Result<Buffer, Errno> {
        let dev = &self.inner.interface.device;
        if !dev.mmap_ok.load(Ordering::Relaxed) {
            return Err(Errno::NOTSUP);
        }
        match Buffer::mmap(&dev.fd, len) {
            Ok(buf) => Ok(buf),
            Err(e) if matches!(e, Errno::PERM | Errno::ACCESS | Errno::INVAL) => {
                dev.mmap_ok.store(false, Ordering::Relaxed);
                debug!(
                    "USB zero-copy mmap unavailable ({e}); using heap buffers"
                );
                Err(e)
            }
            Err(e) => {
