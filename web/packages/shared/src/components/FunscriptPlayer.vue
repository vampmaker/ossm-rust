...
import { OSSM_CONTROL_KEY } from '../control'
...
const injected = inject(OSSM_CONTROL_KEY)
if (!injected) {
  throw new Error('FunscriptPlayer requires OssmControl')
}
...
async function startPlayback() {
  ...
  try {
    ...
    await control.subscribeState(onStreamState, 200)
...
async function stopPlayback() {
  ...
  try {
    if (streamStateListener) {
      await control.unsubscribeState(streamStateListener)
...
function resyncStream(reset = false) {
  ...
  if (chunk.length > 0) {
    void control.sendWaypoints(chunk, reset)
  }
}
...
function feedWaypoints() {
  ...
  if (chunk.length > 0) {
    void control.sendWaypoints(chunk, false)
  }
}
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-md flex flex-wrap items-center justify-between gap-4 transition-all">
      ...
      <div class="flex items-center gap-2.5">
        <span class="text-xs font-bold text-gray-700">{{ t('funscript.speed') }}</span>
        <select
    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-md space-y-4 transition-all">
      <h3 class="text-sm font-extrabold text-gray-800 tracking-tight">{{ t('funscript.strokeLimits') }}</h3>
