...
export class MacroPlayerEngine {
  ...
  async stop(): Promise<void> {
    ...
    try {
      const updated = await this.control.setPaused({ paused: true })
  ...
  private async execute(inst: MacroInstruction): Promise<void> {
    ...
    try {
      if (inst.action === 'start') {
        const updated = await this.control.setPaused({ paused: false })
        this.applyPostedConfig(updated)
        return
      }
      if (inst.action === 'stop') {
        ...
        const updated = position !== undefined
          ? await this.control.setPaused({ paused: true, position })
          : await this.control.setPaused({ paused: true })
        this.applyPostedConfig(updated)
        return
      }
      // set
      ...
      const posted = await this.control.setConfig(domainToWireConfig(this.liveConfig))
